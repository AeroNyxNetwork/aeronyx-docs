 ...
 *
 * Last Modified: v1.0.1 - v0.1 protocol status fallback copy
...
const COPY = {
  en: {
    summary: 'AeroNyx is an open privacy protocol and product ecosystem for private routing, encrypted communication, encrypted storage, Memory Chain, Rust privacy nodes, signed peer discovery, two-hop path proof, blind relay foundations, anonymous access credentials, nodeboard operations, and encrypted agent-to-agent services.',
  ...
  'zh-Hans': {
    summary: 'AeroNyx 是开源隐私协议与产品生态，覆盖隐私网络、端到端加密通信、加密存储、Memory Chain、Rust 隐私节点、签名节点发现、two-hop 路径证明、盲转发基础、匿名访问凭证、nodeboard 运营以及 AI agent 加密连接服务。',
  ...
  'zh-Hant': {
    summary: 'AeroNyx 是開源隱私協議與產品生態，覆蓋隱私網絡、端對端加密通訊、加密儲存、Memory Chain、Rust 隱私節點、簽名節點發現、two-hop 路徑證明、盲轉發基礎、匿名存取憑證、nodeboard 營運以及 AI agent 加密連接服務。',
  ...
  ja: {
    summary: 'AeroNyx は、プライベートルーティング、エンドツーエンド暗号化通信、暗号化ストレージ、Memory Chain、Rust プライバシーノード、署名付きノード発見、two-hop path proof、ブラインドリレー基盤、匿名アクセス資格情報、nodeboard 運用、AI agent 間の暗号化サービスを支えるオープンなプライバシープロトコルです。',
  ...
  ko: {
    summary: 'AeroNyx는 프라이버시 네트워크, 종단 간 암호화 통신, 암호화 저장소, Memory Chain, Rust 프라이버시 노드, 서명된 노드 발견, two-hop path proof, 블라인드 릴레이 기반, 익명 접근 자격 증명, nodeboard 운영, AI agent 간 암호화 서비스를 위한 오픈 프라이버시 프로토콜입니다.',
  ...
  ru: {
    summary: 'AeroNyx — открытый протокол приватности для приватной маршрутизации, сквозного шифрования, зашифрованного хранения, Memory Chain, Rust-узлов, подписанного обнаружения peers, two-hop path proof, blind relay, анонимных учетных данных, nodeboard и шифрованных сервисов для AI agents.',
  ...
  es: {
    summary: 'AeroNyx es un protocolo abierto de privacidad para routing privado, comunicación cifrada de extremo a extremo, almacenamiento cifrado, Memory Chain, nodos Rust, descubrimiento firmado de nodos, two-hop path proof, blind relay, credenciales anónimas, nodeboard y servicios cifrados entre AI agents.',
...
- Rust privacy nodes already report signed peer discovery, peer store recovery, restart recovery, and aggregate peer lifecycle events.
- Public network statistics expose two-hop path proof readiness, blind relay runtime counters, and protocol health without route IDs, endpoints, receiver identities, payloads, client IPs, DNS, Memory Chain plaintext, or social graph edges.
