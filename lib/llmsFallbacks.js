 ...
 * Modification Reason:
 *   v1.3.1 - [DOCS-NODE-EVIDENCE 2026-08-31 by Codex] Refresh recovery and
 *     relay-smoke test evidence after commander integration while preserving
 *     the unverified end-to-end smoke and fleet boundaries.
 ...
 *
 * Last Modified: v1.3.1 - Refresh scoped integration-test evidence
...
const EVIDENCE_BULLETS = {
  en: [
    ...
    'Automated tests verified: 35 targeted relay tests, five foundational recovery tests, six replica-recovery store tests, and three relay-smoke privacy-diagnostic tests passed; socket-bound authenticated end-to-end relay-smoke remains unverified.',
  ...
  'zh-Hans': [
    ...
    '自动化测试已验证：35 个定向中继、5 个基础恢复、6 个副本恢复存储及 3 个 relay-smoke 隐私诊断测试已通过；需要 UDP socket 的认证端到端冒烟仍未验证。',
  ...
  'zh-Hant': [
    ...
    '自動化測試已驗證：35 個定向轉送、5 個基礎復原、6 個副本復原儲存及 3 個 relay-smoke 隱私診斷測試已通過；需要 UDP socket 的認證端到端冒煙仍未驗證。',
  ...
  ja: [
    ...
    'Automated tests verified：対象 relay 35、基礎 recovery 5、replica-recovery store 6、relay-smoke privacy diagnostic 3 tests が合格。UDP socket を使う authenticated end-to-end smoke は未検証です。',
  ...
  ko: [
    ...
    'Automated tests verified: targeted relay 35개, 기초 recovery 5개, replica-recovery store 6개, relay-smoke privacy diagnostic 3개 tests가 통과했습니다. UDP socket 기반 authenticated end-to-end smoke는 미검증입니다.',
  ...
  ru: [
    ...
    'Automated tests verified: прошли 35 targeted relay, 5 базовых recovery, 6 replica-recovery store и 3 relay-smoke privacy-diagnostic tests; authenticated end-to-end smoke с UDP socket не проверен.',
  ...
  es: [
    ...
    'Automated tests verified: pasaron 35 tests relay, 5 tests base de recovery, 6 replica-recovery store y 3 relay-smoke privacy-diagnostic tests; el authenticated end-to-end smoke con UDP socket sigue sin verificar.',
